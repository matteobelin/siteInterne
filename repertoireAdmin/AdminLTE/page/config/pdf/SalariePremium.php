...
$requete =('SELECT salarie.nom as nom,salarie.prenom as prenom,agence.ville as ville,licence.nom as licence FROM salarie join agence on agence.id=salarie.id_agence left join licence on licence.id=salarie.id_licence where licence.nom="Premium" order by nom');
...
class PDF extends FPDF {
  ...
  $pdf->Cell(75,6,'Liste Salarie Premium',0,0,'L',1);    
...
// on affiche les en-têtes du tableau
    ...
    while ($salarie = mysqli_fetch_array($result)){
        ...
        $pdf->SetX(10);
        $pdf->MultiCell(40,8,$salarie['nom'],1,'C'); // 40 -> x, 8 -> y, C = centré # multicell -> Autant de cellules que nécessaire sont générées, les unes en dessous des autres.
        $pdf->SetY($position_detail);
        $pdf->SetX(50); 
...
mysqli_free_result($result);
    $pdf->Output('SalariePremium.pdf','I');?>