...
if(!empty($_POST['search'])){
    $query = $connexion->prepare('SELECT imprimante.marque as marque,imprimante.modele as modele,imprimante.numero_serie as numero_serie,imprimante.ip as ip,salarie.nom as nom, salarie.prenom as prenom FROM imprimante left join salarie on salarie.id=imprimante.id_salarie where salarie.nom like :salarie_nom or salarie.prenom like :salarie_prenom or imprimante.marque like :marque or numero_serie like :numero_serie or imprimante.ip like :ip or imprimante.modele like :modele order by nom');        
    //prepare la requete et selectionne la marque de l'imprimante,le modele, le numero de serie,son ip,a quel salarie elle appartient(NOM,PRENOM) quand le nom du salari, le prenom, la maruqe de l'imprimante,son ip ,son numero de serie ou son modele corresponde a la recherche
    $query->execute([
    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item">
        ...
        <div class="navbar-search-block">
          <form class="form-inline" method="post"><!-- formulaire qui permet de faire une recherche -->
        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          <!-- Add icons to the links using the .nav-icon class
            ...
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a href="listeOrdinateur.php?page=<?php echo $_SERVER['REQUEST_URI']?>" class="nav-link"><!-- $_SERVER['REQUEST_URI'] recupère l'url de la fonction -->
          ...
          <li class="nav-item">
            <a href="../config/pdf/Imprimante.php" class="nav-link">
      ...
      <div class="container-fluid">
        <div class="row">
          <div class="col-12">
            <div class="card">
              <?php
                if(empty($_POST['search'])){// si pas de recherche en cours lance cette requete
                    $statement = $connexion->query('SELECT imprimante.marque as marque,imprimante.modele as modele,imprimante.numero_serie as numero_serie,imprimante.ip as ip,salarie.nom as nom, salarie.prenom as prenom FROM imprimante left join salarie on salarie.id=imprimante.id_salarie order by nom');
                    //recupere la marque de l'imprimante,son modele,son numero de serie,son ip et le salarie a qui elle appartient (NOM,PRENOM)
              ...
              <div class="card-body">
                <table id="example2" class="table table-bordered table-hover">
                        ...
                        <?php foreach($results as $imprimante): ?>
                        <tr>
                            <td scope="col" class="col-1"><?php echo $imprimante['nom']?></td>
                            <td scope="col" class="col-1"><?php echo $imprimante['prenom']?></td>
                            <td scope="col" class="col-1"><?php echo $imprimante['numero_serie']?></td>
                            <td scope="col" class="col-2"><?php echo $imprimante['modele']?></td>
                            <td scope="col" class="col-1"><?php echo $imprimante['marque']?></td>
                            <td scope="col" class="col-1"><?php echo $imprimante['ip']?></td>
                        </tr>
                    <?php endforeach ?>
                </table>
              </div>
