...
$connexion=connect_db();//conncexion a la base de donnée
$statement = $connexion->query('SELECT count(id) FROM demande WHERE resolu=0');// recupere le nombre de demande 
...
if(!empty($_POST['search'])){
    $query = $connexion->prepare('SELECT salarie.nom as nom,salarie.prenom as prenom,agence.ville as ville, licence.nom as licence FROM salarie join agence on agence.id=salarie.id_agence left join licence on licence.id=salarie.id_licence where licence.nom="Premium" and (salarie.nom like :salarie_nom or salarie.prenom like :salarie_prenom or agence.ville like :ville) order by ville');        
    //prepare la selection du nom, prenom du salarié,sa licence et son agence par rapport a la recherche cad si le nom,le prenom ou la ville correspond a la recherche
    $query->execute([
  ...
  // execute la requete avec les valeur qui se trouve dans execute
...
<head>
  ...
  <title>Liste Salarié Premium</title>
...
<div class="wrapper">
  ...
  <nav class="main-header navbar navbar-expand navbar-white navbar-light">
    ...
    <ul class="navbar-nav">
      ...
      <?php if($_SESSION['admin']['accesDemande']==1):?><!-- affihce seulement si la personne a les droits -->
      <li class="nav-item d-none d-sm-inline-block">
        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          <!-- Add icons to the links using the .nav-icon class
            ...
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a href="listeOrdinateur.php?page=<?php echo $_SERVER['REQUEST_URI']?>" class="nav-link"><!-- $_SERVER['REQUEST_URI'] permet de récuperer l'url de la page-->
          ...
          <li class="nav-item">
            <a href="../config/pdf/SalariePremium.php" class="nav-link">
...
<div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Salarié Premium</h1>
          </div><!-- /.col -->
    <!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    ...
    <?php
            $statement = $connexion->query('SELECT licence.nom as licence FROM licence');
            // recupere toute les licences
          ...
          <div class="col-md-12">
              ...
              <div class="card-body">
                <div id="actions" class="row">
                  ...
                  <form style="margin-left: 3em; " method="POST" action="listeSalarie.php" enctype="multipart/form-data"><!-- formaulaire qui permet de filtrer par rapport a la licence-->
                    
                          <select class="form-control select2bs4 m-4" style="width: 100%;" name="licence" id="licence">
                            <option value="liste<?php echo 'Premium'?>.php"><?php echo 'Premium'?></option>
                            <option value="liste<?php echo 'Salarie'?>.php"><?php echo 'Salarié'?></option>
                          <?php foreach($resultat as $result):?>
                            <?php if($result['licence']!='Premium'):?>
            ...
            <div class="card">
              <?php
                if(empty($_POST['search'])){// si pas de recherche effectué 
                    $statement = $connexion->query('SELECT salarie.nom as nom,salarie.prenom as prenom,agence.ville as ville,licence.nom as licence FROM salarie join agence on agence.id=salarie.id_agence left join licence on licence.id=salarie.id_licence where licence.nom="Premium" order by ville');
                    // selection le nom,prenom,la licence et l'agence du salarié
