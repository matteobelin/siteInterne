...
if(!empty($_POST['licence'])):?>
  ...
  <script>
      window.location.href="/repertoireAdmin2/AdminLTE/page/liste/<?php echo $_POST['licence']?>?page=<?php echo $_SERVER['REQUEST_URI'].'?page='.$_SESSION['page'] ?>"
  </script>
<?php endif?>
<?php
if(!empty($_GET['page'])){
...
if(!empty($_POST['search'])){
  $query = $connexion->prepare('SELECT salarie.nom as nom,salarie.prenom as prenom,agence.ville as ville, licence.nom as licence FROM salarie join agence on agence.id=salarie.id_agence left join licence on licence.id=salarie.id_licence where licence.nom like :licence_nom or salarie.nom like :salarie_nom or salarie.prenom like :salarie_prenom or agence.ville like :ville order by ville');        
  $query->execute([
      'licence_nom'=>$_POST['search'].'%' ,
    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            <a href="../config/pdf/Salarie.php" class="nav-link">
          ...
          <div class="col-md-12">
              ...
              <div class="card-body">
                <div id="actions" class="row">
                  ...
                  <form style="margin-left: 3em; " method="POST" action="listeSalarie.php" enctype="multipart/form-data">
        ...
        <div class="row">
          <div class="col-12">
            <div class="card">
              <?php
                if(empty($_POST['search'])){
                    $statement = $connexion->query('SELECT salarie.nom as nom,salarie.prenom as prenom,agence.ville as ville,licence.nom as licence FROM salarie join agence on agence.id=salarie.id_agence left join licence on licence.id=salarie.id_licence  order by ville');
