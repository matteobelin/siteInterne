...
if($_SESSION['admin']['accesTablette']==0)://si pas acces?>
    <script>alert("Vous n'avez pas acces a la modification de tablette")</script>
    <script>
...
$statement = $connexion->query('SELECT count(id) FROM demande WHERE resolu=0');//nombre de demande non resolu
...
foreach($results as $result){
  $count=$result;
    ...
    $statement = $connexion->query('SELECT numero_serie FROM tablette');//numeros de serie des tablettes
    $results = $statement->fetchAll(PDO::FETCH_ASSOC);
  $total=0;

  if(!empty($_GET['numero_serie'] )){
    $_SESSION['admin']['tablette']=$_GET['numero_serie'];
}
if(!empty($_POST['numero_serie'])){
    $_SESSION['admin']['tablette']=$_POST['numero_serie'];
  }

  foreach($results as $numero){
      if($_SESSION['admin']['tablette']==$numero['numero_serie'])//verif si existe
      $total+=1;
  }
  ...
  if(empty( $_SESSION['admin']['tablette'])){
    $errors['errors_numero_serie'] = 'Le numero de serie est obligatoire';
}
  if (! empty($errors)){
      header('Location: modifierTablette.php?'.http_build_query(array_merge($errors,$_POST)));
      return;
  }
  ...
  $statement = $connexion->query('SELECT marque,type,code_compta,id_salarie from tablette where numero_serie="'.$_SESSION['admin']['tablette'].'"');
  //recup marque type code compta id salarie de la tablette
        $results = $statement->fetchAll(PDO::FETCH_ASSOC);

        if(!empty($_SESSION['admin']['tablette']))
        {
            $_GET['numero_serie']=$_SESSION['admin']['tablette'];
            foreach($results as $tablette){
                $_GET['marque']=$tablette['marque'];
                $_GET['type']=$tablette['type'];
                $_GET['code_compta']=$tablette['code_compta'];
            }
            //get->affiche directement dans le form
        }
    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            <a href="modifierTablette.php" class="nav-link">
              ...
              <form action="updateTablette.php" method="post">
                <div class="card-body">
                  <div class="form-group">
                    ...
                    <?php if (isset($_GET['errors_numero_serie']))://gestion erreur ?>
                        <div style='color : red'>
                  ...
                  <div class="form-group">
                    ...
                    <?php if (isset($_GET['errors_code_compta'])): //gestion erreur?>
                        <div style='color : red'>
                  ...
                  <div class="form-group">
                    ...
                    <?php if (isset($_GET['errors_marque']))://gestion erreur ?>
                        <div style='color : red'>
                  ...
                  <div class="form-group">
                    ...
                    <?php if (isset($_GET['errors_type']))://gestion erreur ?>
                        <div style='color : red'>
                  ...
                  <a  class='btn btn-danger' href="delTabletteSalarie.php?numero_serie=<?php echo $_SESSION['admin']['tablette']?>">Supprimer de l'utilisateur</a>
                </form>
